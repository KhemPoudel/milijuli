 ...
 * @property PurchasesInfo[] $purchasesInfos
 * @property InventoryItemsManualIncrement[] $inventoryItemsManualIncrements
 */
class InventoryItems extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('sp, cp,', 'numerical', 'integerOnly'=>true),
			array('item_name', 'length', 'max'=>50),
			array('item_identifier', 'length', 'max'=>20),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, item_name, item_identifier, sp, cp', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'purchasesInfos' => array(self::HAS_MANY, 'PurchasesInfo', 'item_id'),
			'inventoryItemsManualIncrements' => array(self::HAS_MANY, 'InventoryItemsManualIncrement', 'inventory_id'),
		);
	}
    ...
    {
        foreach($this->inventoryItemsManualIncrements as $c)
        {
            $c->delete();
        }
