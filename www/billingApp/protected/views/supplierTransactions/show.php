    ...
    <div id="printable_view">
        ...
        <div>
            <table>
                ...
                <?php
                $opening_models=PurchasesInvoices::model()->findAll(array('condition'=>'supplier_id=:supplier AND issue_date<:fromDate',
                    'params'=>array(':supplier'=>$supplier,':fromDate'=>$fromDate)));
                $opening_cash_models=MoneyPaid::model()->findAll(array('condition'=>'supplier_id=:supplier AND paid_date<:fromDate',
                    'params'=>array(':supplier'=>$supplier,':fromDate'=>$fromDate)));
                $opening_balance=0;
                ...
                { ?>
                    <tr>
                        ...
                        <td class="left-bordered bottom-bordered right-bordered">
                            <?php
                            if($opening_balance<0){
                                echo -1*$opening_balance.'Dr';
                            }
                            else
                                echo $opening_balance.'Cr';
                            ?>
                        </td>
                    </tr>
                <?php }
                $models=PurchasesInvoices::model()->findAll(array('condition'=>'supplier_id=:supplier AND issue_date<=:toDate AND issue_date>=:fromDate',
                    'params'=>array(':supplier'=>$supplier,':toDate'=>$toDate,':fromDate'=>$fromDate)));
