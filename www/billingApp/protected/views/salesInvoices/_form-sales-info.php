<div style="position: relative;" class="row-inline well" id="<?php echo 'div_form_content_',$sts; ?>">
    ...
    if($sts===0){
        ...
        <div class="row row2">
            <?php
                echo $form->labelEx($model2,'qty');
            ?>
            <?php
            if(!empty($errors[$sts]))
                echo $form->textField($model2,"[$sts]qty",array('class'=>$sts,'style'=>'color:red;border:2px yellow solid;'));
            else
                echo $form->textField($model2,"[$sts]qty",array('class'=>$sts));
            ?>
...
$('#add-row-btn').bind('click',function(){
    ...
    function eventBind(count){
        ...
        btnId='#'+count;
        uid='#SalesInfo_'+count+'_unit_price';
        qid='#SalesInfo_'+count+'_qty';
        did='#SalesInfo_'+count+'_discount';
        tid='#SalesInfo_'+count+'_total_amount';
    ...
    function scopepreserver(divCount)
    {
            ...
            var tidIndv='#SalesInfo_'+divCount+'_total_amount';
            var uidIndv='#SalesInfo_'+divCount+'_unit_price';
            var qidIndv='#SalesInfo_'+divCount+'_qty';
            var didIndv='#SalesInfo_'+divCount+'_discount';
            interval=setInterval(function(){
                ...
                for(var i=0;i<=count;i++)
                {
                    var elem=document.getElementById('SalesInfo_'+i+'_total_amount');
                    if(elem==null)
                        indVal=0;
                    else
                        indVal=elem.value;
                ...
                $('#SalesInvoices_total_amount').val(totalAmount);
            },1);

    }
    var totalAmount=$('#SalesInvoices_total_amount').val();
    function diffTotal(removedDiv)
    {
        totalAmount-=$('#SalesInfo_'+removedDiv+'_total_amount').val();
        $('#SalesInvoices_total_amount').val(totalAmount);
    }
});
